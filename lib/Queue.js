...
export default class Queue extends EventEmitter {
  constructor(name, options = {}) {
    ...
    this.opts = {
      ...
      options: {},
      client: null,
      errorQueueName: `${this.name}_error`,
      ...options,
    };
    ...
    if (this.opts.client !== null && this.opts.client.constructor.name === 'RedisClient') {
      ...
      this.client = Redis.createClient(this.opts.port, this.opts.host, {
        ...this.opts.options,
        retry_strategy: (log) => {
          if (log.error.code === 'ENOTFOUND' || log.error.code === 'ECONNREFUSED') {
            this.emit('error', log.error);
            return undefined;
          }
          if (log.times_connected > 10) {
            this.emit('error', log.error);
            return undefined;
          }
          return 3000;
  ...
  getAllErrors() {
    return new Promise((resolve, reject) => {
      this.client.lrange(this.opts.errorQueueName, 0, -1, (err, n) => {
        if (err) return reject(err);
        return resolve(n);
      });
    });
  ...
  requeue(payload) {
    return new Promise((resolve, reject) => {
      this.client.rpush([this.opts.errorQueueName, payload], (err, reply) => {
        if (err) return reject(err);
        return resolve(reply);
      });
    });
